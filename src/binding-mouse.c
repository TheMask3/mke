...
/*
 * @overload set(surface, x, y) 
 *   Create a new mouse cursor
 *
 *   @param [SDL::Surface] surface the surface for new cursor icon
 *   @param [Integer] x the x coordinate within the window
 *   @param [Integer] y the y coordinate within the window
 *   @return [SDL::Cursor] the new cursor created
 */
static VALUE Cursor_s_new(VALUE self, VALUE surface, VALUE x, VALUE y)
{
    SDL_Cursor *cur = SDL_CreateColorCursor(Get_SDL_Surface(surface), x, y);
    return wrapObject(cur, SDL_SurfaceType);
}
 ...
 *
 *   @param [SDL::Cursor] cursor the cursor to be selected
 *   @return [nil]
 */
static VALUE Cursor_s_set(VALUE self, VALUE surface, VALUE x, VALUE y)
{
    SDL_Cursor *cur = SDL_CreateColorCursor(Get_SDL_Surface(surface), x, y);
    SDL_SetCursor(cur);
    return Qnil;
...
#endif
    rb_define_singleton_method(cCursor, "new_cursor", Cursor_s_new, 4);
    rb_define_singleton_method(cCursor, "cursor=", Cursor_s_set, 2);
