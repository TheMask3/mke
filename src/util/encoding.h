...
static std::string getCharset(std::string &str) {
    ...
    if (ret.empty())
        throw Exception(Exception::MKEError, "Could not detect string encoding", str.c_str());
    return ret;
}
...
static std::string convertString(std::string &str) {
    ...
    if (result != (size_t)-1 && errno == 0)
    {
        ...
        throw Exception(Exception::MKEError, "Unable to convert string (Guessed encoding: %s)", charset.c_str());
    }
